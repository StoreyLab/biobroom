...
author: "Andrew Bass, Emily Nelson and David Robinson"
...
This package contains methods for converting standard objects constructed by
bioinformatics packages, especially those in BioConductor, and converting them
to tidy data. It thus serves as a complement to the *broom* package, and
follows the same the `tidy`/`augment`/`glance` division of tidying methods.
Tidying data makes it easy to recombine, reshape and visualize bioinformatics
analyses.

*biobroom* implements tidying methods for both S3 and S4 classes. Objects that
can be tidied include

* ExpressionSet objects (from *Biobase*)
* MSnSet objects (from *MSnbase*)
* per-gene differential expression tests from *limma*, *edgeR*, *edge*, and *DESeq2*
* *qvalue* multiple hypothesis testing objects

We are currently working on adding additional methods to existing Bioconductor
objects. Please visit our [github page](https://github.com/StoreyLab/biobroom)
if you would like to add new tidying methods for objects.
...
`qobj` is a list and in order to use popular packages like `dplyr` or `ggplot`,
we need to convert the results to a data frame. To make things worse, the elements
in the list are different lengths. The `biobroom` package makes this conversion easy
by using the `tidy`, `augment` and `glance` functions:
...
Applying these functions to `qobj` created earlier,  
...
The gene names can be inputted into `augment` using the `gene.names` argument.
...
Additionally, we can easily extract out important information such as significant
