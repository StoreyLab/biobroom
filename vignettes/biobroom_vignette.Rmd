...
"Tidy data" is a data analysis paradigm that focuses on keeping data in so-called "long" format--essentially a single observation per row of a data table. For further information, please see Hadley Wickham's [paper](http://vita.had.co.nz/papers/tidy-data.pdf) on the subject.
...
Next, we can calculate q-values and merge the object with `tidy_results` using the `augment` function:

```{r}
library(dplyr)

qobj <- qvalue(tidy_results$p.value)

qt_results <- augment(qobj, data = tidy_results)
head(qt_results)
```
