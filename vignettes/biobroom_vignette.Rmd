...
`qobj` is a list and if we wanted to use popular package, such as `dplyr` or `ggplot`,
...
`airway_se` is a `SummarizedExperiment` object. Next, we create a `DESeqDataSet` object
...
`tidy_result`, we can use `ggplot2` to create a volcano plot of the p-values: 
...
Using `hammer` data included in `biobroom` package and performing differential
expression analysis:
```{r}
library(edgeR)
data(hammer)
...
Generate some random data to test the tidier for `limma` objects:
...
Use `lmFit` and `eBayes` to fit a linear model and use `tidy` to convert the
...
`tidy` can also be run directly on `ExpressionSet` objects from `Biobase.` The
`hammer` dataset we used above is an `ExpressionSet` object, so we'll use that to demonstrate.
